...
/  ___| |           | | | | |   | ___ \ |   (_)   | |              
\ `--.| |_ ___  __ _| | |_| |__ | |_/ / |__  _ ___| |__   ___ _ __ 
...
def customfol():
    ...
    while True:
        ...
        if has_files == "y":
            ...
            sprint(f"\n{info}Contact TheBizarreAbhishek")
            bgtask("xdg-open https://t.me/TheBizarreAbhishek")
            pexit()
...
def requirements():
    ...
    if "linux" in platform:
        if "arm64" in architecture or "aarch64" in architecture:
            ...
            if not isloclx:
                download("https://github.com/TheBizarreAbhishek/StealthSites/releases/download/tunnelers/loclx-linux-arm64.zip", "loclx.zip")
            ...
            if not isloclx:
                download("https://github.com/TheBizarreAbhishek/StealthSites/releases/download/tunnelers/loclx-linux-arm.zip", "loclx.zip")
            ...
            if not isloclx:
                download("https://github.com/TheBizarreAbhishek/StealthSites/releases/download/tunnelers/loclx-linux-amd64.zip", "loclx.zip")
        else:
            if not iscloudflared:
            ...
            if not isloclx:
                download("https://github.com/TheBizarreAbhishek/StealthSites/releases/download/tunnelers/loclx-linux-386.zip", "loclx.zip")
        ...
        if "x86_64" in architecture or "amd64" in architecture:
            ...
            if not isloclx:
                download("https://github.com/TheBizarreAbhishek/StealthSites/releases/download/tunnelers/loclx-darwin-amd64.zip", "loclx.zip")
            ...
            if not isloclx:
                download("https://github.com/TheBizarreAbhishek/StealthSites/releases/download/tunnelers/loclx-darwin-arm64.zip", "loclx.zip")
        else:
    ...
    if isfile(f"{sites_dir}/version.txt"):
        ...
        # download(websites_url, "StealthSites.zip")
        print(f"\n{info2}Downloading website files....{nc}")
        shell(f"git clone {sites_repo} {sites_dir}")
        # shell(f"cd {sites_dir} && git pull")
    if isfile("StealthSites.zip"):
        extract("StealthSites.zip", ".tempdir")
        delete("StealthSites.zip")
...
def main_menu():
    ...
    while True:
        ...
        if choice in choices:
            ...
            bgtask("xdg-open 'https://github.com/TheBizarreAbhishek")
        elif choice == "0":
            pexit()
        else:
...
def secondary_menu(sites, name):
    ...
    if customdir is None:
        ...
        if not isdir(site):
            internet()
            delete("site.zip")
            download(f"https://github.com/TheBizarreAbhishek/StealthSites/raw/main/{folder}.zip", "site.zip")
